...
static void generate(const struct parser_dag *dag, FILE *file)
{
	...
	// fprintf(file, "return sigmoid(t%d);\n}\n", dag->id);  // if want to use sigmoid function, then uncomment this line
}

typedef double (*evaluate_t)(void); // function pointer type
// typedef double (*sigmoid_t)(void);	// function pointer type
...
int main(int argc, char *argv[])
{
	...
	generate(parser_dag(parser), file); // parse_dag give the root node
	parser_close(parser);
	fclose(file);
	...
	if (!(jitc = jitc_open(SOFILE)) ||
		!(fnc = (evaluate_t)jitc_lookup(jitc, "evaluate"))) // fnc is a pointer to function
	{
		file_delete(SOFILE);
		jitc_close(jitc);
		TRACE(0);
	...
	printf("Finally the answer is: %f\n", fnc());
