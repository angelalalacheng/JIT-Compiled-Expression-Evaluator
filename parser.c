...
static void free_dag(struct parser_dag *dag)
{
	if (dag)
	{
		free_dag(dag->left);
...
static const struct lexer_token *next(const struct parser *parser)
{
...
static int												   /* BOOL */
match(const struct parser *parser, enum lexer_token_op op) // dag token match the lexer
{
...
static void forward(struct parser *parser)
{
	if (parser->i < parser->n)
	{
		++parser->i;
...
static struct parser_dag *expr_primary(struct parser *parser)
{
	struct parser_dag *dag;
...
static struct parser_dag *expr_unary(struct parser *parser) // test whether it is umnary operation
{
	struct parser_dag *dag;
	...
	{
		forward(parser); // forward i (current token index)to next token
	...
	else if (match(parser, LEXER_OP_SUB))
	{
		...
		forward(parser);
		if (!(dag->right = expr_unary(parser))) // '-' 'number' -> '-' is unary operation
		{
			TRACE_ONCE(parser, "invalid unary '-' operand");
			return NULL;
		}
...
static struct parser_dag *expr_multiplicative_(struct parser *parser, struct parser_dag *left)
{
...
static struct parser_dag *expr_multiplicative(struct parser *parser)
{
	struct parser_dag *dag;
...
static struct parser_dag *expr_additive_(i parser *parser, struct parser_dag *left)
{
	const char *const TBL[] = {"+", "-"};
	struct parser_dag *dag;
	char buf[64];
...
static struct parser_dag *expr_additive(struct parser *parser)
{
	struct parser_dag *dag;
...
static struct parser_dag *top(struct parser *parser)
{
	struct parser_dag *dag;
...
{
	struct parser *parser; // declare a parser
	...
	lexer_close(parser->lexer); // no longer needed
	parser->lexer = NULL;
	return parser;
}
